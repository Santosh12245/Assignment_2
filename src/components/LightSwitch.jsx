...
const LightSwitch = () => {
    ...
    return (
<div className={`min-h-screen rounded-xl transition-colors duration-500 p-8 flex flex-col items-center justify-center ${isLightOn ? 'bg-amber-50' : 'bg-slate-950'}`}>

            {/* --- DYNAMIC HEADING SECTION --- */}
            <div className="text-center mb-12 relative">
                <h1 className={`text-4xl md:text-6xl font-black tracking-tighter transition-all duration-500 ${isLightOn
                        ? 'text-slate-900'
                        : 'text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-600 drop-shadow-[0_0_15px_rgba(251,191,36,0.3)]'
                    }`}>
                    Interactive State Terminal
                </h1>

                <div className="flex items-center justify-center gap-4 mt-3">
                    <div className={`h-[2px] w-12 rounded-full transition-colors duration-500 ${isLightOn ? 'bg-amber-200' : 'bg-slate-800'}`}></div>
                    <p className={`uppercase tracking-[0.4em] text-[10px] font-bold transition-colors duration-500 ${isLightOn ? 'text-amber-600' : 'text-slate-500'}`}>
                        Dual-Context Control Unit
                    </p>
                    <div className={`h-[2px] w-12 rounded-full transition-colors duration-500 ${isLightOn ? 'bg-amber-200' : 'bg-slate-800'}`}></div>
                </div>
            </div>
